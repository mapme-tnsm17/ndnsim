...
#include "%ssrc/ndnSIM/ndn-cxx/src/security/cryptopp.hpp"
#include <iostream>
...
def check_cryptopp(self, *k, **kw):
    ...
    for defines in [[], ['CRYPTOPP_DISABLE_ASM']]:
        try:
            # When passing -o, the include path for cryptopp.hpp in the fragment
            # is wrong. We fix this manually, eg.
            #   self.bldnode.abspath() = /home/jordan/bin/ndn-systemx/ndn-cxx/build
            #   os.getcwd() = /home/jordan/bin/ndn-systemx/ndn-cxx
            import os
            path_depth = len(self.bldnode.abspath().split(os.sep)) - len(os.getcwd().split(os.sep)) + 1
            self.check_cxx(msg='Checking if CryptoPP library works',
                           fragment=CRYPTOPP_CHECK_FRAGMENT % ('../' * path_depth),
                           lib='cryptopp',
