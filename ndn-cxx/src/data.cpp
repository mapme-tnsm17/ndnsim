...
Data::wireEncode(EncodingImpl<TAG>& encoder, bool unsignedPortion/* = false*/) const
{
  ...
  //            MetaInfo
  //            Content
#ifdef NDN_CXX_PATH_LABELLING
  //            pathID
#endif // NDN_CXX_PATH_LABELLING
  //            Signature
  // (reverse encoding)
...
#ifdef NDN_CXX_HOP_COUNT
  /*
   * Code added by Mauro (msardara@cisco.com)
   */
  totalLength +=  prependNonNegativeIntegerBlock(encoder, tlv::HopCount, hop_count);
#endif // NDN_CXX_HOP_COUNT


#ifdef NDN_CXX_PATH_LABELLING
  if(!m_pathId.empty())
    totalLength += encoder.prependBlock(getPathId());
#endif // NDN_CXX_PATH_LABELLING
...
Data::wireDecode(const Block& wire)
{
...
#ifdef NDN_CXX_PATH_LABELLING
  if(m_wire.find(PATH_ID_TYPE)==m_wire.elements_end()) {
    m_pathId.reset();
  }
  else {
    m_pathId = m_wire.get(PATH_ID_TYPE);
  }
#endif // NDN_CXX_PATH_LABELLING
   ...
   */
  // HopCount
  Block::element_const_iterator val_hopCount;
  if ((val_hopCount = m_wire.find(tlv::HopCount)) != m_wire.elements_end())
    hop_count = static_cast<unsigned int>(readNonNegativeInteger(*val_hopCount));
  else
    hop_count = 0;
#endif
...
#ifdef NDN_CXX_PATH_LABELLING
const Block&
Data::getPathId() const
{
  if (m_pathId.empty())
#ifdef NDNSIM
    m_pathId = makeEmptyBlock(PATH_ID_TYPE);
#else
  m_pathId = dataBlock(PATH_ID_TYPE, reinterpret_cast<const uint8_t*>(0), 0);
#endif // NDNSIM
  if (!m_pathId.hasWire())
      m_pathId.encode();
  return m_pathId;
...
bool
Data::isPathIdEmpty() const
{
  return m_pathId.empty();
}

Data&
Data::setPathId(const uint8_t* pathId, size_t pathIdLength)
{
  onChanged();

#ifdef NDNSIM
  m_pathId = makeBinaryBlock(PATH_ID_TYPE, pathId, pathIdLength);
#else
  m_pathId = dataBlock(PATH_ID_TYPE, pathId, pathIdLength);
#endif // NDNSIM
...
Data&
Data::setPathId(const Block& pathId)
{
  onChanged();

  if (pathId.type() == PATH_ID_TYPE)
    m_pathId = pathId;
  else
    m_pathId = Block(PATH_ID_TYPE, pathId);

  return *this;
}

Data&
Data::setPathIdAndWire(uint8_t pathId)
{

  if (m_pathId.empty())
    std::cout<< "ERROR: pathID is empty\n";
  else {
    uint8_t* id = const_cast<uint8_t*>(m_pathId.value());
    *id = pathId;
  }
 ...
 */
unsigned int
Data::getHopCount() const
{
  return hop_count;
}

void
Data::incrementHopCount()
{
  onChanged();
  hop_count++;
}
