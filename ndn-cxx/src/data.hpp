...
// Assign the path id type with number of 76 to avoid type contention,
// Check the NDN type
#define PATH_ID_TYPE 76
...
public:
...
#ifdef NDN_CXX_PATH_LABELLING
  /**
   * @brief Get path ID Block
   *
   * To access path ID value, one can use value()/value_size() or
   * value_begin()/value_end() methods of the Block class
   */
  ...
  /**
   * @brief check if path ID is empty
   */
  ...
  /**
   * @brief Set the path ID from the block
   *
   * Depending on type of the supplied block, there are two cases:
   *
   * - if block.type() == Path ID type, then block will be used directly as Data packet's
   *   content (no extra copying)
   *
   * - if block.type() != path Id type, then this method will create a new Block with type
   *   tlv::Content and put block as a nested element in the content Block.
   *
   * @param block The Block containing the path ID to assign
   *
   * @return This Data so that you can chain calls to update values.
   */
  Data&
  setPathId(const Block& block);
  
  /**
   * @brief set the path ID to a specific value so that the wire format is also updated  
   * accordingly in an efficient way.
   * 
   * @return This Data so that you can chain calls to update values.
   */
  Data&
  setPathIdAndWire(uint8_t pathId);

  /**
   * @brief Set the path ID from the buffer (buffer will be copied)
   *
  ...
  Data&
  setPathId(const uint8_t* buffer, size_t bufferSize);
#endif // NDN_CXX_PATH_LABELLING
...
#ifdef NDN_CXX_PATH_LABELLING
  //path ID field definition
  mutable Block m_pathId;
#endif // NDN_CXX_PATH_LABELLING
     ...
     * Code added by Mauro (msardara@cisco.com)
     */
    mutable unsigned int hop_count = 0;
#endif // NDN_CXX_HOP_COUNT
#endif // !NDNSIM
