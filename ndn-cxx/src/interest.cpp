...
#ifdef NDN_CXX_MAPME
  , m_specialInterestType(tlv::TYPE_UNDEFINED)
  , m_specialInterestSequenceNumber(-1)
#endif // NDN_CXX_MAPME
#ifdef NDN_CXX_KITE
  , m_traceForwardingFlag(255)
#endif // NDN_CXX_KITE
{
}
...
Interest::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  if (getSpecialInterestType() >= 0) {
    totalLength += prependNonNegativeIntegerBlock(encoder,
                                                  tlv::SpecialInterestType,
                                                  getSpecialInterestType());
  }
#endif // NDN_CXX_MAPME
...
#ifdef NDN_CXX_KITE
  // TraceName
  if(!getTraceName().empty())
    totalLength += prependNestedBlock(encoder,tlv::TraceName,getTraceName());

  // TraceForwardingFlag
  if (getTraceForwardingFlag() != 255)
    totalLength += prependNonNegativeIntegerBlock(encoder, tlv::TraceForwardingFlag, getTraceForwardingFlag());
#endif // NDN_CXX_KITE
...
Interest::wireDecode(const Block& wire)
{
 ...
 #ifdef NDN_CXX_KITE
  // TraceForwardingFlag
  val = m_wire.find(tlv::TraceForwardingFlag);
  if (val != m_wire.elements_end())
    m_traceForwardingFlag = readNonNegativeInteger(*val);
  else
    m_traceForwardingFlag = 255;
  
  val = m_wire.find(tlv::TraceName);
  if (val != m_wire.elements_end()) {
    (*val).parse();
    m_traceName.wireDecode((*val).get(tlv::Name));
  }
  else
    m_traceName.clear();
#endif // NDN_CXX_KITE
...
#ifdef NDN_CXX_MAPME 
  val = m_wire.find(tlv::SpecialInterestType);
  if (val != m_wire.elements_end()) {
    m_specialInterestType = static_cast<tlv::SpecialInterestTypeValue>(readNonNegativeInteger(*val));
  }
  else{
    m_specialInterestType = tlv::TYPE_UNDEFINED;
  }

  val = m_wire.find(tlv::SequenceNumber);
  if (val != m_wire.elements_end())
    m_specialInterestSequenceNumber = readNonNegativeInteger(*val);
  else{
    m_specialInterestSequenceNumber = -1;
  }
...
#ifdef NDN_CXX_MAPME
  if (interest.getSpecialInterestType() != tlv::TYPE_UNDEFINED) {
    os << delim << "ndn.SpecialInterestType=" << interest.getSpecialInterestType();
    delim = '&';
  }
  if (interest.getSequenceNumber() >=0) {
    os << delim << "ndn.SequenceNumber=" << interest.getSequenceNumber();
    delim = '&';
  }
#endif // NDN_CXX_MAPME

#ifdef NDN_CXX_KITE
  os << delim << "ndn.TraceForwardingFlag=" << interest.getTraceForwardingFlag();
  delim = '&';
  os << delim << "ndn.TraceName=" << interest.getTraceName();
#endif  // NDN_CXX_KITE
