...
#ifdef PATH_LABELLING
const boost::random::uniform_int_distribution<> FaceTable::dist(0,255);
#endif // PATH_LABELLING
...
#ifdef PATH_LABELLING
  FaceId randomId = dist(getGlobalRng());
  FaceId faceId = ++m_lastFaceId;
  faceId = (faceId << 8) + randomId;
#else
  FaceId faceId = ++m_lastFaceId;
#endif
...
FaceTable::remove(shared_ptr<Face> face, const std::string& reason)
{
  ...
  // XXX UGLY ! In addition to have an accessor, this should be selective !
  m_forwarder.getFib().removeNextHopFromAllEntries(face, m_forwarder.m_removeFibEntries);
#else
