...
public: // triggers
  ...
  void
  SetNRetransmissions(uint32_t n_rtx);
...
//#endif
  void
  AddFaceToJustUsed(shared_ptr<Face> face); // Should be supported by all strategies.
  	  	  	  	  	  	  	  	  	  	  	// Don't forget to call this function right after the sending of an Interest
  //Natalya
...
//--------------------Natalya
inline const Fib&
Strategy::getFIB() const
{
  return m_forwarder.getFib();
}

inline void
Strategy::SetNRetransmissions(uint32_t n_rtx)
{
	std::cout<<this<<" SetRetransmissions "<<n_rtx<<std::endl;;
	m_MaxRtxAllowed = n_rtx;
	std::cout<<this<<" SetRetransmissions end "<<this->m_MaxRtxAllowed<<std::endl;;

}

inline uint32_t
Strategy::GetNRetransmissions()
{
	return m_MaxRtxAllowed;
}
