...
#ifdef FIB_EXTENSIONS
class FibExtensionData {
public:
  /* virtual is required to let the destructor be manipulated by shared_ptr<> */
  virtual ~FibExtensionData() {};
};
...
#ifdef FIB_EXTENSIONS
public:
  uint64_t getSeq() { return m_seq; }
  void setSeq(uint64_t seq) { m_seq = seq; }
  shared_ptr<FibExtensionData> getExtension() { return m_extension; }
  void setExtension(shared_ptr<FibExtensionData> extension) { m_extension = extension; }

   /**
    * \brief remove all the nexhops from the nexhop list of this tfib
    * entry. (a convinient function for updating a tfib entry)
    */
  void
  removeNextHops();
...
private:
  uint64_t m_seq;
  shared_ptr<FibExtensionData> m_extension;
#endif // FIB_EXTENSIONS
...
#ifdef FIB_EXTENSIONS
inline void
Entry::removeNextHops()
{
    m_nextHops.clear();
#if 0
    // XXX temporarily done in FibManager
    _onUpdate(this);
#endif
}
#endif // FIB_EXTENSIONS
