...
public: // signals
  /** \brief fires after a fib::Entry is added
   */
  signal::Signal<Fib, shared_ptr<fib::Entry>> onAdd;

  /** \brief fires before a fib::Entry is removed
   */
  signal::Signal<Fib, shared_ptr<fib::Entry>> onRemove;

  /** \brief fires after a fib::Entry is updated
   */
  signal::Signal<Fib, shared_ptr<fib::Entry>> onUpdate;

  void 
  _onUpdate(shared_ptr<fib::Entry>);
...
public: // mutation
  ...
  void
#ifdef FIB_EXTENSIONS
  removeNextHopFromAllEntries(shared_ptr<Face> face, bool clear_empty = true);
#else
